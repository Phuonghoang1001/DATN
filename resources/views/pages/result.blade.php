                    ...
                    <div class="col-md-9">
                        <div class="inner test-single">
                            <div class="row">
                                <div class="test-header clearfix">
                                        ...
                                        <form action="tests/{!! $lesson_item->id !!}/load_question.html" method="GET"
                                            ...
                                            <label>
                                                <input type="radio" name="level" id="medium"
                                                       value="medium" @if($level == 'medium'){!! "checked " !!}@endif>Trung
                                                bình
                                            </label>
                                            ...
                                            <a href="test/{!! $lesson_item->id !!}.html"
                                               class="pull-right btn-refresh">@if(count($correct) < 4 ) {!! 'Làm lại <i class="fa fa-refresh" aria-hidden="true"></i>' !!} @else {!! 'Level khác <i class="fa fa-angle-double-right" aria-hidden="true"></i>' !!} @endif</a>
                                        </form>
                                    </div>
                                    <div class="col-md-5 test-header-right">
                                        ...
                                        <div class="result-test">Điểm của bạn : <strong>{!! count($correct) !!}
                                                / {!!  count($question_test)!!} </strong> @if(count($correct) < 4 ) {!! 'Lêu lêu' !!} @else {!! 'vỗ tay' !!} @endif
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            @if(!empty($question_test))
                                <form action="" method="POST" enctype="multipart/form-data" id="form-result">
                                    <input type="hidden" name="_token" value="{{csrf_token()}}">
                                    <?php $num = 0; ?>
                                        ...
                                        <div class="question-item">
                                            ...
                                            <label>
                                                ...
                                                @if(isset($wrong["{$item->id}"]) && $answer["{$item->id}"] == $wrong["{$item->id}"] && $answer["{$item->id}"] == $item->answer_1 )
                                                    <span style="color:red;"><i class="fa fa-times"
                                                                                aria-hidden="true"></i> {!! $item->answer_1 !!}</span>
                                                @else
                                                    {!! $item->answer_1 !!}
                                                @endif
                                            </label>
                                            <label>
                                                ...
                                                @if(isset($wrong["{$item->id}"]) && $answer["{$item->id}"] == $wrong["{$item->id}"] && $answer["{$item->id}"] == $item->answer_2 )
                                                    <span style="color:red;"><i class="fa fa-times"
                                                                                aria-hidden="true"></i> {!! $item->answer_2 !!}</span>
                                                @else
                                                    {!! $item->answer_2 !!}
                                                @endif
                                            </label>
                                            <label>
                                                ...
                                                @if(isset($wrong["{$item->id}"]) && $answer["{$item->id}"] == $wrong["{$item->id}"] && $answer["{$item->id}"] == $item->answer_3 )
                                                    <span style="color:red;"><i class="fa fa-times"
                                                                                aria-hidden="true"></i> {!! $item->answer_3 !!}</span>
                                                @else
                                                    {!! $item->answer_3 !!}
                                                @endif
                                            </label>
                                            <label>
                                                ...
                                                @if(isset($wrong["{$item->id}"]) && $answer["{$item->id}"] == $wrong["{$item->id}"] && $answer["{$item->id}"] == $item->answer_4 )
                                                    <span style="color:red;"><i class="fa fa-times"
                                                                                aria-hidden="true"></i> {!! $item->answer_4 !!}</span>
                                                @else
                                                    {!! $item->answer_4 !!}
                                                @endif
                                            </label><br>
